...
(defun %compute-repl-frame ()
  ...
  (let ((lines '())
    ...
    (format nil
            ";; Restricted Lisp REPL. Reply with ONE s-expression, nothing else.~%;; Choose the call that directly advances the user request. Never repeat a completed call.~%;; Use EXACTLY these calls, with real arguments (no &key, no &optional):~%~{~A~%~};; Complete action-to-answer forms:~%~{~A~%~};; ANSWER FROM MEMORY — the MOST IMPORTANT form. If YOUR MEMORY above already contains the~%;; facts, do NOT call recall again: read them, REASON, and respond in your OWN words:~%;;   (respond \"the build order is Vesta, then Helios, then Luna, then Tethys\")~%;;   (respond \"no — that would create a dependency cycle\")~%;;   (respond \"Helios depends on Vesta\")~%;; COMBINE several facts when the question needs it — an order, a plan, a yes/no with a reason,~%;; a conclusion drawn from multiple memories. The answer is plain text inside (respond \"...\").~%;; ARITHMETIC EVALUATES — compute directly, do NOT store the unevaluated text:~%;;   (* 4 5) => 20    (+ 36 30) => 66    (- 36 6) => 30    (/ 90 3) => 30~%;; To answer a calculation, compute INSIDE respond: (respond (* 4 5 3)) => 60~%;; The numbers come from YOUR MEMORY above (and results below) — read them and put the~%;; literal numbers into the call. To keep a computed value for later, store it as a fact:~%;;   (store \"SQ is 36\")   then a later turn:   (recall \"SQ\")~%;; MULTI-STEP: one call per round; the previous result is the ';;' line above — use it, then (respond ...).~%"
            (nreverse lines)
            (nreverse answers))))
