    ...
    (dolist (e edges)
      (let ((a (getf e :a))
        ...
        (when (or (search "tool" a :test #'char-equal)
                  ...
                  (member a '("provider-router" "git-ops" "memory") :test #'string=)
                  (member b '("provider-router" "git-ops" "memory") :test #'string=))
          (incf total)
...
(defun %vitruvian-scores (global local projection logistic lorenz map)
  (let* ((strength (%clamp (+ (* (harmony-policy-number "vitruvian/strength-chaos-weight" 0.6) (- 1.0 (getf logistic :chaos-risk)))
                           ...
                           0.0 1.0))
         (sv-weight (harmony-policy-number "vitruvian/utility-supervision-weight" 0.20))
         (supervision (%supervision-rate))
         (utility (%clamp (+ (* (harmony-policy-number "vitruvian/utility-global-weight" 0.35) (getf global :global-score))
                             (* (harmony-policy-number "vitruvian/utility-coherence-weight" 0.25) (%tool-memory-coherence map))
                             (* (harmony-policy-number "vitruvian/utility-balance-weight" 0.20) (getf (%complexity-balance map) :balance))
                             (* sv-weight supervision))
                          0.0 1.0))
    ...
    (case phase
      ...
      (:stabilize
       ...
       (when (and (fboundp 'memory-field-port-ready-p)
                  (funcall 'memory-field-port-ready-p))
         (ignore-errors
           (let ((checkpoint (ipc-call (%sexp-to-ipc-string
                                        '(:component "memory-field" :op "checkpoint")))))
             (when (and checkpoint (ipc-reply-ok-p checkpoint))
               (ignore-errors
