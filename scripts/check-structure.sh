...
done < <(find "$REPO_DIR/harmonia" "$REPO_DIR/harmoniislib" "$REPO_DIR/OS4-iOS" "$REPO_DIR/OS4-Android" -type f \( -name '*.rs' -o -name '*.lisp' -o -name '*.swift' -o -name '*.kt' -o -name '*.cpp' \) -print0)
