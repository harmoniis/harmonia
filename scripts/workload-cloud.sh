...
echo "[3/4] Live S3 upload via libharmonia_s3.dylib"
...
HARMONIA_S3_MODE=aws sbcl --disable-debugger \
  ...
  --eval '(cffi:load-foreign-library #P"/Users/george/harmoniis/projects/agent/harmonia/target/release/libharmonia_s3.dylib")' \
  --eval '(cffi:defcfun ("harmonia_s3_upload_file" up) :int (src :string) (bucket :string) (prefix :string) (key :string))' \
