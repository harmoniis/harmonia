...
cargo build --release -p harmonia-http -p harmonia-recovery -p harmonia-rust-forge -p harmonia-fs -p harmonia-browser -p harmonia-push-sns -p harmonia-cron-scheduler
...
HARMONIA_FS_ROOT="$FSROOT" HARMONIA_RECOVERY_LOG="$RECLOG" HARMONIA_PUSH_SNS_MODE=log HARMONIA_PUSH_SNS_LOG="$PUSHLOG" sbcl --disable-debugger \
  ...
  --eval '(cffi:load-foreign-library #P"/Users/george/harmoniis/projects/agent/harmonia/target/release/libharmonia_push_sns.dylib")' \
  ...
  --eval '(cffi:defcfun ("harmonia_push_sns_send" psend) :int (arn :string) (subject :string) (message :string))' \
  ...
  --eval '(unless (zerop (psend "arn:aws:sns:us-east-1:000000000000:harmonia-test" "harmonia" "push-log-test")) (sb-ext:exit :code 9))' \
