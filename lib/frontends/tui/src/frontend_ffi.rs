...
// Version
// ---------------------------------------------------------------------------
const VERSION: &[u8] = b"harmonia-tui/0.1.0\0";
...
pub extern "C" fn harmonia_frontend_init(config: *const c_char) -> i32 {
    let _config = match cstr_to_string(config) {
        Ok(v) => v,
        Err(e) => {
            set_error(e);
            return -1;
...
pub extern "C" fn harmonia_frontend_last_error() -> *const c_char {
    let s = last_error()
        .read()
        .map(|v| v.clone())
        .unwrap_or_else(|_| "tui lock poisoned".into());
    CString::new(s)
...
mod tests {
    ...
    fn test_version() {
        ...
        assert_eq!(v, "harmonia-tui/0.1.0");
    }
}
