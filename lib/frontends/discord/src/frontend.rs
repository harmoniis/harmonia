//! [`DiscordFrontend`] — actor-owned Discord messaging frontend.
...
#[async_trait]
impl Frontend for DiscordFrontend {
    type Config = String;
    ...
    async fn init(config: String) -> Result<Self, String> {
        ...
        let bot_token = read_vault_secret(DISCORD_BOT_TOKEN_SYMBOLS)?.unwrap_or_default();
        ...
        if channels.is_empty() {
            return Err(
                "missing channels: set (channels ...) or config-store discord-frontend/channels"
                    .into(),
            );
        }

        let last_message_ids = channels.iter().map(|c| (c.clone(), String::new())).collect();
        Ok(Self {
            bot_token,
            channels,
            last_message_ids,
    ...
    async fn poll(&mut self) -> PollResult {
        ...
        for channel in self.channels.clone() {
            let cursor = self.last_message_ids.get(&channel).cloned().unwrap_or_default();
            let mut messages = if cursor.is_empty() {
                list_messages(&self.bot_token, &channel, None)?
                    .into_iter()
                    .take(1)
                    .collect::<Vec<_>>()
            } else {
                list_messages(&self.bot_token, &channel, Some(&cursor))?
            };
            ...
            if max_seen > previous {
                self.last_message_ids
                    .insert(channel.clone(), max_seen.to_string());
            }
        }
