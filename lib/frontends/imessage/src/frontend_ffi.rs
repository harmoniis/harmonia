...
// Version
// ---------------------------------------------------------------------------
const VERSION: &[u8] = b"harmonia-imessage/0.1.0\0";
...
pub extern "C" fn harmonia_frontend_send(channel: *const c_char, payload: *const c_char) -> i32 {
    ...
    match client::send(&channel, &payload) {
        Ok(()) => {
            clear_error();
            0
        }
...
pub extern "C" fn harmonia_frontend_last_error() -> *const c_char {
    let s = last_error()
        .read()
        .map(|v| v.clone())
        .unwrap_or_else(|_| "imessage lock poisoned".into());
    CString::new(s)
...
mod tests {
    ...
    fn test_version() {
        ...
        assert_eq!(v, "harmonia-imessage/0.1.0");
    }
}
