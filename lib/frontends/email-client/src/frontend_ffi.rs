...
const VERSION: &[u8] = b"harmonia-email-client/0.2.0\0";
...
pub extern "C" fn harmonia_frontend_send(channel: *const c_char, payload: *const c_char) -> i32 {
    let to = match cstr_to_string(channel) {
        Ok(v) => v,
        Err(e) => {
            set_error(e);
            return -1;
    ...
    let text = match cstr_to_string(payload) {
        Ok(v) => v,
        Err(e) => {
            set_error(e);
            return -1;
...
mod tests {
    ...
    fn test_healthcheck_before_init() {
        // Before init, healthcheck returns 0
    ...
    fn test_version() {
        ...
        assert_eq!(v, "harmonia-email-client/0.2.0");
    }
}
