...
use crate::runtime::{
    ...
    register_node, report, route_allowed, route_allowed_with_context, route_timeseries,
    set_last_error, set_store, set_tool_enabled, time_report,
};
...
#[no_mangle]
pub extern "C" fn harmonia_harmonic_matrix_route_allowed_with_context(
    from: *const c_char,
    to: *const c_char,
    signal: f64,
    noise: f64,
    ...
    match route_allowed_with_context(&from, &to, signal, noise, security_weight, dissonance) {
        Ok(true) => {
            clear_last_error();
            1
        }
