//! Chronicle record operations — write events to the knowledge base.

use harmonia_actor_protocol::{extract_sexp_bool, extract_sexp_string};

use super::super::{dispatch_op, param, param_f64, param_u64};
    ...
    Some(match op {
        ...
        "record-memory-event" => dispatch_op!("record-memory-event", {
            ...
            harmonia_chronicle::memory::record(
                ...
                old_size, new_size, node_count, edge_count,
                interdisciplinary_edges, max_depth, detail.as_deref(),
            ).map(|_| "(:ok)".to_string())
        }),
        ...
        "update-access" => dispatch_op!("update-access", {
            let id = param!(sexp, ":id");
            harmonia_chronicle::memory::update_access(&id).map(|_| "(:ok)".to_string())
        }),
        "record-delegation" => dispatch_op!("record-delegation", {
            ...
            harmonia_chronicle::delegation::record(
                task_hint.as_deref(), &model_chosen, &backend, reason.as_deref(),
                escalated, escalated_from.as_deref(), cost_usd, latency_ms,
        ...
        "record-signalograd-event" => dispatch_op!("record-signalograd-event", {
            ...
            harmonia_chronicle::signalograd::record(
                ...
                accepted, recall_hits, checkpoint_path.as_deref(),
                checkpoint_digest.as_deref(), detail.as_deref(),
            ).map(|_| "(:ok)".to_string())
        }),
        "record-phoenix-event" => dispatch_op!("record-phoenix-event", {
            let event_type = param!(sexp, ":event-type");
            let exit_code: Option<i32> = extract_sexp_string(sexp, ":exit-code").and_then(|s| s.parse().ok());
            let attempt: Option<i32> = extract_sexp_string(sexp, ":attempt").and_then(|s| s.parse().ok());
            let max_attempts: Option<i32> = extract_sexp_string(sexp, ":max-attempts").and_then(|s| s.parse().ok());
            let recovery_ms: Option<i64> = extract_sexp_string(sexp, ":recovery-ms").and_then(|s| s.parse().ok());
            let detail = extract_sexp_string(sexp, ":detail");
            harmonia_chronicle::phoenix::record(
                &event_type, exit_code, attempt, max_attempts, recovery_ms, detail.as_deref(),
            ).map(|_| "(:ok)".to_string())
        }),
        "record-ouroboros-event" => dispatch_op!("record-ouroboros-event", {
            ...
            let detail = extract_sexp_string(sexp, ":detail");
            let patch_size: Option<i64> = extract_sexp_string(sexp, ":patch-size").and_then(|s| s.parse().ok());
            let success = extract_sexp_bool(sexp, ":success").unwrap_or(false);
            harmonia_chronicle::ouroboros::record(
                &event_type, component.as_deref(), detail.as_deref(), patch_size, success,
            ).map(|_| "(:ok)".to_string())
        }),
