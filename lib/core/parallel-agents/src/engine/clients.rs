...
pub(super) fn request_openrouter(prompt: &str, model: &str, api_key: &str) -> Result<String, String> {
    let payload = format!(
...
fn request_exa(query: &str, api_key: &str) -> Result<String, String> {
    let endpoint =
        env::var("HARMONIA_EXA_API_URL").unwrap_or_else(|_| "https://api.exa.ai/search".to_string());
