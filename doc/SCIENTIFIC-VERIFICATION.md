...
- **D2 well-posedness (Phase 5) — DONE** (Rust, `lib/tools/dynamics-verify`): a forced Lorenz `ρ(t)=28+8·sin(0.3t)` is **non-Markov** in `(x,y,z)`. The autonomous `(x,y,z)` basis — DYSCO's Markov frame — gives **dynR²(ẏ)=0.799** (blind to the forcing); adding the forcing "view" `x·sin(0.3t)` recovers it at **dynR²=1.000**. So the forced/emergent law is well-posed as **compression** once the basis carries the forcing — a concrete demonstration that the multi-view/forcing 'view' DYSCO has and harmonia lacks (comparison §3.2/§7) is exactly what autonomous Markov identification is missing. No Markov assumption is needed for the compression question.
- **End-to-end probes (Phase 6) — PARTIAL** via the continuous runner `scripts/dev/verify-campaign.sh`. **Offline GREEN:** dynamics-verify 4/4 + **193 core Rust tests** (memory-field 42+88, signalograd, mempalace, chronicle …) pass. **Live-agent circuits/memory-field + resonance: BLOCKED** — this sandbox has no reachable LLM provider (orchestration → `IPC unreachable`); the operator brings the full stack up safely via `scripts/dev/harness.sh bringup` (pid-tracked, never touches the live state root). An agent-only deep-probe run scored 13/38 — every miss was `IPC unreachable`/`FIELD-LOAD-FIDELITY :UNREACHABLE` (no runtime up), i.e. environmental, not a regression.
- **Least-action self-rewrite loop (Phase 8) — DONE (offline logic verified).** `src/core/equation-identify.lisp` + transcendental REPL primitives (`sin`/`cos`/`sqrt`/`exp`/… so the agent can eval transcendental governing laws). The loop: identify the minimal-description-length law from a readout `Θ·Ξ` → emit s-expr → eval through the **real** `%reval` → least-action descent of one bounded policy parameter. Verified **11/11 offline** (`tests/test-equation-identify.lisp` via `scripts/dev/run-equation-identify.sh`, in the continuous runner): the agent identifies AND evals its own Lorenz and *transcendental Thomas* law (`(+ (* 1.0 (sin y)) (* -0.19 x))` → 0.81), descends a bounded parameter (1.0→0.905), reports a Solomonoff prior, and no-ops when the harmonic gate is closed. **Live seam (env-blocked):** wiring into `harmonic-machine.lisp`'s `:rewrite-plan` needs signalograd's readout `Θ` via IPC + a config-store write — achievable when the runtime is up; the loop LOGIC is verified here against the agent's own evaluator.
