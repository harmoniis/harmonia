...
- Changes:
  ...
  - Poll format standardized as 3-field tab-separated triples: `sub_channel\tpayload\tmetadata_sexp`.
...
- Risk Notes: 3-field poll format is the standard format; gateway provides defaults for simpler frontends.
- Rollback Plan: Revert capabilities/metadata fields to None.
...
- Law/Principle Applied: Single-responsibility dispatch; boundary-first safety; Lisp orchestrates, Rust executes.
- Changes:
  - Gateway `command_dispatch.rs`: single interception point for ALL /commands from ALL frontends (TUI, MQTT, Telegram, Tailscale, paired nodes).
...
- Risk Notes: Lisp command callback must be registered before any frontend sends commands. Gateway init happens before frontend registration.
- Rollback Plan: Remove command interception from poll_baseband; remove callback registration from init-baseband-port; restore %maybe-dispatch-system-command as primary handler.

## v11 — 2026-03-12

- Target: Resumable sessions, default deny-all sender policy for messaging frontends, TUI policy management.
- Motivation: Users need to browse and resume past sessions. Messaging frontends must not accept signals from arbitrary senders — deny-by-default enforces the security-kernel principle at the gateway boundary.
- Law/Principle Applied: Deny-by-default (security kernel principle); boundary-first safety; user sovereignty over communication channels.
- Changes:
  - **Resumable Sessions** (`cli/paths.rs`, `cli/session.rs`, `cli/menus.rs`):
    - `SessionSummary` struct and `list_sessions(label)` enumerate all past sessions with event counts and timestamps.
    - `format_timestamp_ms(ms)` formats epoch millis as local `YYYY-MM-DD HH:MM` without chrono.
    - `write_current_session` made public for session switching.
    - `/resume` TUI command shows interactive menu of past sessions sorted by most recent. On selection, writes `current-session` and exits for reconnect.
    - "Resume Session" added to main menu (`/menu`).
  - **Gateway Sender Policy** (`lib/core/gateway/src/sender_policy.rs` — NEW):
    - `MESSAGING_FRONTENDS` constant: email, slack, discord, mattermost, signal, whatsapp, imessage, telegram, nostr.
    - `SenderPolicyCache` with per-frontend allowlists and allow-all sets, lazy-loaded from config-store with 30s TTL.
    - `is_signal_allowed(envelope)`: pass-through for non-messaging/self-originated, check allowlist/allow-all, default deny.
    - `reload_policies()` for forced cache refresh.
    - Filter applied in `poll_baseband()` before command interception.
    - Config-store access: gateway component granted read access to `sender-policy` scope via `default_extra_read_scopes`.
  - **TUI Policy Management** (`cli/session.rs`, `cli/menus.rs`):
    - `/policies` command and "Policies" submenu with per-frontend sender management (list/add/remove/allow-all/deny-all).
    - Config-store keys: `sender-policy/allowlist-<frontend>` (comma-separated), `sender-policy/mode-<frontend>` (`deny`/`allow-all`).
  - **Config-Store Policy** (`lib/core/config-store/src/policy.rs`):
    - `gateway` component granted read access to `sender-policy` scope.
- Risk Notes: Sender policy is additive — existing signals from TUI/MQTT/Tailscale are unaffected. Messaging frontends with no allowlist configured will silently drop all signals (deny-by-default). Users must configure allowlists via `/policies` for each messaging frontend they want to receive from.
- Rollback Plan: Remove sender policy filter from poll_baseband; delete sender_policy.rs; remove `/resume` and `/policies` from session.rs; remove gateway extra read scope.
